...
function showHideRules() {
  ...
  crossBtn.style.display =
    crossBtn.style.getPropertyValue("display") === "inline-block" ? "none" : "inline-block";
}
...
Array.from(selectedItem).forEach((btn) => {
  btn.addEventListener("click", (e) => {
    ...
    winnerCheck(e.currentTarget.getAttribute("value"), pcChoice[random]);
    play.classList.add("inactive");
    resultSection.classList.remove("inactive");
  });
});

function winnerCheck(player, computer) {
  if (player == "rock") {
    if (computer == "rock") {
...
const playerScoreDisplay = document.querySelector("#pScore_number");
const pcScoreDisplay = document.querySelector("#cScore_number");
